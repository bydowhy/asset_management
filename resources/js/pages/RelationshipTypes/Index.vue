...
function destroy(type: any) {
    if (!confirm(`Hapus relationship type "${type.name}"?`)) return;
    router.delete(`/relationship-types/${type.id}`, { preserveScroll: true });
}
</script>
...
<template>
    ...
    <div class="flex h-full flex-1 flex-col gap-4 rounded-xl p-4">
        <div class="flex items-center justify-between">
            <h1 class="text-2xl font-bold">Relationship Types</h1>
            <Link href="/relationship-types/create">
                <Button size="sm">+ New Relationship Type</Button>
            </Link>
        </div>
                ...
                <TableBody>
                    <TableRow v-for="type in types" :key="type.id">
                        ...
                        <TableCell class="text-center">
                            <Badge variant="secondary">{{ type.asset_relationships_count }}</Badge>
                        </TableCell>
                        <TableCell class="text-right">
                            <div class="flex justify-end gap-2">
                                <Link :href="`/relationship-types/${type.id}/edit`">
